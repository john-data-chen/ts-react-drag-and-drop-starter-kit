...
const TodoCard = memo(({ todo, toggleComplete, deleteTodo, handleEditTodo }: TodoCardProps) => {
  ...
  const formattedDate = useMemo(() => {
    return todo.dueDate ? new Date(todo.dueDate).toLocaleDateString() : 'None'
  }, [todo.dueDate])
    ...
    >
      <span className={todo.completed ? 'completedTask' : ''}>
        ...
        <span className="dueDateWrapper">
          <h4 className="dueDateTitle" data-testid="dueDateTitle" aria-label={dueDateText}>
            {dueDateText}
          </h4>
          <p className="dueDate fixLongText" data-testid="dueDate" aria-label={formattedDate}>
            {formattedDate}
          </p>
        </span>
      </span>
